...
export interface TeamAnnouncement {
  ...
  role: string;
  date: string;
  category: 'General' | 'Engineering' | 'Marketing' | 'SLA Operations' | 'Personnel' | 'Corporate';
...
export function AdminPanel({ onExit }: AdminPanelProps = {}) {
  ...
  const [staff, setStaff] = useState<Employee[]>(() => {
    ...
    const needsMigration = stored.length > 0 && (!stored.some(s => s.permissions) || !stored.some(s => s.role.includes('Project Management')));
    ...
    if (!hasNewStaff || stored.length <= 3 || needsMigration) {
      const initialStaff: Employee[] = [
        { 
          ...
          role: 'Founder | Managing Director | International Business & Growth Leadership', 
        ...
        { 
          id: 'st_muntaha', 
          name: 'Muntaha Sheikh', 
          role: 'CEO | Co-Founder & Chief Technical Officer | System Developer and Project Management', 
        ...
        { 
          ...
          role: 'Co-Founder | System Designer & Lead IT Engineer', 
  ...
  const [submissions, setSubmissions] = useState<ContactSubmission[]>(() => getStored('mwi_contact_submissions', [
    {
      ...
      project_details: 'Seeking a localized compliance ledger with offline replication models to track medical asset distribution securely under healthcare data privacy constraints.',
      status: 'Pending',
  ...
  const [emails, setEmails] = useState<HostingerEmail[]>(() => getStored('mwi_emails', [
    ...
    },
    {
      ...
      body: 'Dear Solutions Team,\n\nI submitted an inquiry through your contact portal earlier. We are looking to build a high-availability, highly secure patient ledger with offline replication. Given MetaWave\'s reputation for offline-first architecture, we would like to schedule a virtual presentation on Friday.\n\nCould you please let me know your availability?\n\nSincerely,\nEleanor Vance\nVP of Products, Cygnus Medical Systems',
  ...
  const [announcements, setAnnouncements] = useState<TeamAnnouncement[]>(() => getStored('mwi_announcements', [
    ...
    {
      id: 'ann2',
      title: 'Executive Leadership Appointment: Muntaha Sheikh as CEO & CTO',
      content: 'Please join us in celebrating Muntaha Sheikh\'s executive appointment as Chief Executive Officer (CEO), Co-Founder & Chief Technical Officer. Muntaha leads executive governance, system developer pipelines, and end-to-end project management across all MetaWave software engineering divisions.',
      author: 'Ali Hassan Chand',
      role: 'Founder | Managing Director',
    ...
    },
    {
      ...
      content: 'Co-Founder, System Designer & Lead IT Engineer Abdul Ahad Arain and the core engineering team have greenlit the next major release of our POS software. It includes optimized ESC/POS print-spooling drivers and responsive UI touch layouts. All operations nodes are instructed to initiate client demo deployment packages.',
  ...
  const [announcementFilter, setAnnouncementFilter] = useState<'All' | 'General' | 'Engineering' | 'Marketing' | 'SLA Operations' | 'Personnel' | 'Corporate'>('All');
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 text-slate-100 flex flex-col justify-between font-mono space-y-4">
                    <div className="space-y-3.5">
                      ...
                      <div className="space-y-1">
                        <label htmlFor="target-server-node-select" className="text-[9.5px] text-slate-400">Target Server Node:</label>
                        <select
                          id="target-server-node-select"
                          aria-label="Target Server Node"
                          value={pingTarget}
                    ...
                    {[
                      {
                        ...
                        exec: "Ali Hassan Chand",
                        desc: "Founder & Managing Director",
                        modules: "All 9 System Modules",
                        badge: "bg-rose-500/10 text-rose-400 border border-rose-500/20",
                        clearance: "Level 5 (Founder)"
                      },
                      {
                        dept: "Executive Management",
                        exec: "Muntaha Sheikh",
                        desc: "CEO, Co-Founder & CTO",
                        modules: "Full Systems, Engineering & Management",
                        badge: "bg-purple-500/10 text-purple-400 border border-purple-500/20",
                        clearance: "Level 5 (CEO)"
                      },
                      {
                        dept: "Systems & Engineering",
                        exec: "Abdul Ahad Arain",
                        desc: "Co-Founder & System Designer",
                        modules: "Products Catalog, SLA Services, Bulletins",
                        badge: "bg-teal-500/10 text-teal-400 border border-teal-500/20",
                        clearance: "Level 4 (Co-Founder)"
                      },
                      {
                        dept: "Growth & Marketing",
                        exec: "Suhail Siyal",
                        desc: "Partner & Director of Marketing",
                      ...
                      },
                      {
                        ...
                        desc: "System Orchestrators & Support",
          ...
          <div className="bg-white border border-slate-200 w-full max-w-md rounded-2xl p-6 shadow-2xl relative z-10 text-left space-y-4 font-sans text-xs">
            ...
            <form onSubmit={saveProduct} className="space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3.5">
                ...
                <div className="space-y-1">
                  <label htmlFor="product-category-select" className="font-bold text-slate-600 block">Category Nodes</label>
                  <select id="product-category-select" aria-label="Category Nodes" value={productForm.category} onChange={(e) => setProductForm({ ...productForm, category: e.target.value })} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 select-none">
          ...
          <div className="bg-white border border-slate-200 w-full max-w-md rounded-2xl p-6 shadow-2xl relative z-10 text-left space-y-4 font-sans text-xs">
            ...
            <form onSubmit={saveClient} className="space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3.5">
                <div className="space-y-1">
                  <label htmlFor="client-status-select" className="font-bold text-slate-600 block">Contract Status</label>
                  <select id="client-status-select" aria-label="Contract Status" value={clientForm.status} onChange={(e) => setClientForm({ ...clientForm, status: e.target.value as any })} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 select-none font-sans">
          ...
          <div className="bg-slate-900 border border-slate-800 text-slate-100 w-full max-w-md rounded-2xl p-6 shadow-2xl relative z-10 text-left space-y-4 text-xs">
            ...
            <form onSubmit={saveSocialPost} className="space-y-3.5">
              <div className="space-y-1">
                <label htmlFor="social-platform-select" className="text-slate-400 block font-bold">Select Platform *</label>
                <select id="social-platform-select" aria-label="Select Platform" value={socialForm.platform} onChange={(e) => setSocialForm({ ...socialForm, platform: e.target.value as any })} className="w-full px-3 py-2 bg-slate-950 border border-slate-800 text-white rounded-xl focus:outline-none font-bold">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 select-none">
          ...
          <div className="bg-white border border-slate-200 w-full max-w-md rounded-2xl p-6 shadow-2xl relative z-10 text-left space-y-4 font-sans text-xs">
            ...
            <form onSubmit={logSale} className="space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3.5">
                <div className="space-y-1">
                  <label htmlFor="sale-product-select" className="font-bold text-slate-650 block">Product</label>
                  <select id="sale-product-select" aria-label="Product" value={saleForm.productName} onChange={(e) => setSaleForm({ ...saleForm, productName: e.target.value })} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-semibold">
                    {products.map((p) => (
                ...
                <div className="space-y-1">
                  <label htmlFor="sale-license-select" className="font-bold text-slate-650 block">License Tier</label>
                  <select id="sale-license-select" aria-label="License Tier" value={saleForm.licenseTier} onChange={(e) => setSaleForm({ ...saleForm, licenseTier: e.target.value })} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold">
          ...
          <div className="bg-white border border-slate-200 w-full max-w-md rounded-2xl p-6 shadow-2xl relative z-10 text-left space-y-4 font-sans text-xs">
            ...
            <form onSubmit={saveStaff} className="space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3.5">
                ...
                <div className="space-y-1">
                  <label htmlFor="staff-department-select" className="font-bold text-slate-600 block">Department *</label>
                  <select id="staff-department-select" aria-label="Department" value={staffForm.department} onChange={(e) => setStaffForm({ ...staffForm, department: e.target.value })} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold">
              ...
              <div className="grid grid-cols-2 gap-3.5">
                ...
                <div className="space-y-1">
                  <label htmlFor="staff-status-select" className="font-bold text-slate-600 block">Status</label>
                  <select id="staff-status-select" aria-label="Status" value={staffForm.status} onChange={(e) => setStaffForm({ ...staffForm, status: e.target.value as any })} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 select-none">
          ...
          <div className="bg-white border border-slate-200 w-full max-w-md rounded-2xl p-6 shadow-2xl relative z-10 text-left space-y-4 font-sans text-xs">
            ...
            <form onSubmit={saveManualInquiry} className="space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3.5">
                <div className="space-y-1">
                  <label htmlFor="inquiry-service-select" className="font-bold text-slate-600 block">Service Required</label>
                  <select id="inquiry-service-select" aria-label="Service Required" value={manualInquiryForm.serviceNeeded} onChange={(e) => setManualInquiryForm({ ...manualInquiryForm, serviceNeeded: e.target.value })} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold">
                ...
                <div className="space-y-1">
                  <label htmlFor="inquiry-budget-select" className="font-bold text-slate-600 block">Budget Scale</label>
                  <select id="inquiry-budget-select" aria-label="Budget Scale" value={manualInquiryForm.budgetRange} onChange={(e) => setManualInquiryForm({ ...manualInquiryForm, budgetRange: e.target.value })} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold">
            ...
            {isSendingSmtp ? (
              ...
              <form onSubmit={sendMailViaHostingerSMTP} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label htmlFor="compose-email-from-select" className="font-bold text-slate-600 block">From Account *</label>
                    <select
                      id="compose-email-from-select"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 select-none">
          ...
          <div className="bg-white border border-slate-200 w-full max-w-md rounded-2xl p-6 shadow-2xl relative z-10 text-left space-y-4 font-sans text-xs">
            ...
            <form onSubmit={createAnnouncement} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3.5">
                <div className="space-y-1">
                  <label htmlFor="announcement-category-select" className="font-bold text-slate-600 block">Category *</label>
                  <select
                ...
                <div className="space-y-1">
                  <label htmlFor="announcement-priority-select" className="font-bold text-slate-600 block">Priority *</label>
                  <select
              ...
              <div className="space-y-1">
                <label htmlFor="announcement-author-select" className="font-bold text-slate-600 block">Author *</label>
                <select
                  id="announcement-author-select"
                  aria-label="Author"
